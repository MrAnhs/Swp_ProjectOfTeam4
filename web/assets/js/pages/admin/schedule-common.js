...
onReady(function () {
    ...
    const unifiedSearch = document.getElementById('unifiedSearchInput') || document.getElementById('unifiedSearch');

    // 4.1 Khởi tạo & Tự động load Lịch tuần
    ...
    // 4.2 Đồng bộ lịch tuần bằng cách click các nút tuần ở Unified Picker
    ...
    if (unifiedPrevWeekBtn && unifiedWeekPicker) {
        unifiedPrevWeekBtn.addEventListener('click', function () {
            ...
            } else if (typeof window.loadWeeklyCalendar === 'function') {
                window.loadWeeklyCalendar();
            }
        });
    }
    ...
    // 4.3 Tìm kiếm nhân sự trực tiếp trên Lịch tuần
    const searchInputEl = document.getElementById('unifiedSearchInput') || unifiedSearch;
    if (searchInputEl) {
    ...
    if (unifiedFilterResetBtn) {
        unifiedFilterResetBtn.addEventListener('click', function () {
            ...
            if (unifiedRoleFilter) unifiedRoleFilter.value = 'Doctor';
            if (unifiedWeekPicker) unifiedWeekPicker.value = new Date().toISOString().slice(0, 10);

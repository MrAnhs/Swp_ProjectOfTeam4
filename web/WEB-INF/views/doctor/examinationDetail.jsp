            ...
            <%@taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
                ...
                <head>
                    ...
                    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css"
                        rel="stylesheet">
                    <link href="${pageContext.request.contextPath}/assets/css/pages/doctor/doctor.css?v=20260721-ui2"
                        rel="stylesheet">
                    <style>
                        .exam-hero {
                            overflow: hidden;
                        ...
                        .lab-request-form .form-select {
                            min-height: 48px;
                            font-size: 1rem;
                        }

                        .lab-request-form .btn {
                            min-height: 48px;
                            font-weight: 800;
                        }
                ...
                <body class="doctor-app">
                    ...
                    <c:set var="hasAIResult"
                        value="${record.status == 'AI_Processed' || record.status == 'Editing' || record.status == 'Completed'}" />
                    ...
                    <%@ include file="/WEB-INF/views/components/doctor/sidebar.jspf" %>
                        ...
                        <main class="doctor-main">
                            ...
                            <section class="doctor-card exam-hero mb-4">
                                <div
                                    class="position-relative d-flex flex-column flex-lg-row justify-content-between gap-3 align-items-lg-center">
                                    <div>
                                        <div class="small opacity-75">BƯỚC 4: KHÁM CHI TIẾT · HỒ SƠ
                                            #${record.healthRecordId}</div>
                                        <h1 class="h3 fw-bold mb-1">${record.patientName}</h1>
                                        <div class="doctor-muted">Bệnh nhân #${record.patientId} · ${patient.age} tuổi ·
                                            ${patient.gender}</div>
                                    </div>
                            ...
                            <nav class="exam-section-nav" aria-label="Các phần của hồ sơ khám">
                                ...
                                <c:if test="${hasCompletedLaboratoryRequest}">
                                    <a href="#examinationResult"><i class="bi bi-activity"></i> Kết quả khám & Chẩn
                                        đoán</a>
                                </c:if>
                            </nav>
                            ...
                            <section id="patientContext" class="doctor-card mb-4">
                                <div class="d-flex flex-column flex-xl-row justify-content-between gap-3 mb-4">
                                    <div class="d-flex align-items-center gap-3">
                                        ...
                                        <div>
                                            <h2 class="doctor-section-title h5 mb-0">Thông tin bệnh nhân và lịch sử hội
                                                thoại</h2>
                                            <div class="doctor-muted small">Thông tin hành chính và nội dung bệnh nhân
                                                đã trao đổi với trợ lý AI.</div>
                                        </div>
                                    </div>
                                ...
                                <div class="patient-info-grid mb-4">
                                    <div class="patient-info-item">
                                        <div class="patient-info-label">Mã bệnh nhân</div>
                                        <div class="patient-info-value">#${patient.patientId}</div>
                                    </div>
                                    <div class="patient-info-item">
                                        <div class="patient-info-label">Họ và tên</div>
                                        <div class="patient-info-value">${patient.fullName}</div>
                                    </div>
                                    <div class="patient-info-item">
                                        <div class="patient-info-label">Tuổi</div>
                                        <div class="patient-info-value">${patient.age} tuổi</div>
                                    </div>
                                    <div class="patient-info-item">
                                        <div class="patient-info-label">Giới tính</div>
                                        <div class="patient-info-value">${patient.gender == 'M' ? 'Nam' :
                                            (patient.gender == 'F' ? 'Nữ' : patient.gender)}</div>
                                    </div>
                                    <div class="patient-info-item wide">
                                        <div class="patient-info-label">Điện thoại</div>
                                        <div class="patient-info-value">${empty patient.phone ? 'Chưa cập nhật' :
                                            patient.phone}</div>
                                    </div>
                                    <div class="patient-info-item wide">
                                        <div class="patient-info-label">Email</div>
                                        <div class="patient-info-value">${empty patient.email ? 'Chưa cập nhật' :
                                            patient.email}</div>
                                    </div>
                                    <div class="patient-info-item wide">
                                        <div class="patient-info-label">Địa chỉ</div>
                                        <div class="patient-info-value">${empty patient.address ? 'Chưa cập nhật' :
                                            patient.address}</div>
                                    </div>
                                    <div class="patient-info-item wide">
                                ...
                                <div class="row g-4">
                                    <div class="col-lg-12">
                                        <h3 class="h6 fw-bold mb-3"><i class="bi bi-stars text-warning"></i> Tóm tắt
                                            triệu chứng từ AI</h3>
                                        <c:choose>
                                            <c:when test="${empty aiConversation || empty aiConversation.aiSummary}">
                                                <div class="chat-summary doctor-muted">Chưa có nội dung triệu chứng được
                                                    tổng hợp từ cuộc trò chuyện ban đầu.</div>
                                            </c:when>
                                            <c:otherwise>
                                                <div class="chat-summary">${aiConversation.aiSummary}</div>
                                                <div class="doctor-muted small mt-2">
                                                    Cập nhật:
                                                    <fmt:formatDate value="${aiConversation.createdAt}"
                                            ...
                                            <table class="table doctor-table align-middle mb-0">
                                                ...
                                                <tbody>
                                                    <c:forEach var="history" items="${medicalHistory}">
                                                        <tr>
                                                            <td>
                                                                <fmt:formatDate value="${history.processedAt}"
                                                                    pattern="dd/MM/yyyy HH:mm" />
                                                            </td>
                                                            <td>${empty history.finalDiagnosis ? 'Chưa có' :
                                                                history.finalDiagnosis}</td>
                                                            <td>${empty history.doctorNote ? 'Chưa có' :
                                                                history.doctorNote}</td>
                                                            <td>
                                                                Đường huyết ${history.hba1c} · Urea ${history.urea} · CR
                                                                ${history.cr}
                                                                <c:if
                                                                    test="${history.diabetesProbability > 0 || history.preDiabetesProbability > 0 || history.normalProbability > 0}">
                                                                    <br>
                                                                    <small class="text-secondary fw-semibold">
                                                                        AI Dự đoán:
                                                                        Tiểu đường (
                                                                        <fmt:formatNumber
                                                                            value="${history.diabetesProbability}"
                                                                            type="percent" />) ·
                                                                        Tiền tiểu đường (
                                                                        <fmt:formatNumber
                                                                            value="${history.preDiabetesProbability}"
                                                                            type="percent" />) ·
                                                                        Bình thường (
                                                                        <fmt:formatNumber
                                                                            value="${history.normalProbability}"
                                                                            type="percent" />)
                                                                    </small>
                                                                </c:if>
                                                            </td>
                                    ...
                                    <div class="col-xl-7">
                                        <section class="doctor-card mb-4">
                                            <div class="d-flex align-items-center gap-3 mb-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="doctor-muted small">Các chỉ số mới nhất từ phòng xét
                                                        nghiệm.</div>
                                                </div>
                                            </div>
                                            <div class="exam-grid">
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">Urea</div><input id="metricUrea"
                                                        class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.urea != null ? record.urea : 'Chưa có'}"
                                                        readonly disabled>
                                                </div>
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">Creatinine</div><input id="metricCr"
                                                        class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.cr != null ? record.cr : 'Chưa có'}" readonly
                                                        disabled>
                                                </div>
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">Đường huyết (HbA1c)</div><input
                                                        id="metricHba1c" class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.hba1c != null ? record.hba1c : 'Chưa có'}"
                                                        readonly disabled>
                                                </div>
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">Cholesterol</div><input
                                                        id="metricChol" class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.chol != null ? record.chol : 'Chưa có'}"
                                                        readonly disabled>
                                                </div>
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">TG</div><input id="metricTg"
                                                        class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.tg != null ? record.tg : 'Chưa có'}" readonly
                                                        disabled>
                                                </div>
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">HDL</div><input id="metricHdl"
                                                        class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.hdl != null ? record.hdl : 'Chưa có'}" readonly
                                                        disabled>
                                                </div>
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">LDL</div><input id="metricIdl"
                                                        class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.ldl != null ? record.ldl : 'Chưa có'}" readonly
                                                        disabled>
                                                </div>
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">VLDL</div><input id="metricVldl"
                                                        class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.vldl != null ? record.vldl : 'Chưa có'}"
                                                        readonly disabled>
                                                </div>
                                                <div class="exam-metric">
                                                    <div class="exam-metric-label">BMI</div><input id="metricBmi"
                                                        class="form-control form-control-sm fw-bold"
                                                        style="color: #34d399 !important;" type="text"
                                                        value="${record.bmi != null ? record.bmi : 'Chưa có'}" readonly
                                                        disabled>
                                                </div>
                                            </div>
                                        </section>
                                                ...
                                                <div
                                                    ...
                                                    <div class="d-flex align-items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div class="doctor-muted small">Công cụ hỗ trợ đánh giá nguy
                                                                cơ.</div>
                                                        </div>
                                                    </div>
                                                    <span class="doctor-badge badge-requested">Tham khảo</span>
                                                </div>
                                                <div class="alert alert-warning"><i class="bi bi-info-circle"></i> Chẩn
                                                    đoán cuối cùng thuộc trách nhiệm của bác sĩ.</div>
                                                <div class="mb-3">
                                                    <div class="d-flex justify-content-between mb-1"><strong>Tiểu
                                                            đường</strong>
                                                        <fmt:formatNumber value="${record.diabetes_probability}"
                                                            type="percent" />
                                                    </div>
                                                    <div class="progress ai-progress">
                                                        <div class="progress-bar bg-danger"
                                                            style="width:${record.diabetes_probability * 100}%"></div>
                                                    </div>
                                                </div>
                                                <div class="mb-3">
                                                    <div class="d-flex justify-content-between mb-1"><strong>Tiền tiểu
                                                            đường</strong>
                                                        <fmt:formatNumber value="${record.pre_diabetes_probability}"
                                                            type="percent" />
                                                    </div>
                                                    <div class="progress ai-progress">
                                                        <div class="progress-bar bg-warning"
                                                            style="width:${record.pre_diabetes_probability * 100}%">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div>
                                                    <div class="d-flex justify-content-between mb-1"><strong>Bình
                                                            thường</strong>
                                                        <fmt:formatNumber value="${record.normal_probability}"
                                                            type="percent" />
                                                    </div>
                                                    <div class="progress ai-progress">
                                                        <div class="progress-bar bg-success"
                                                            style="width:${record.normal_probability * 100}%"></div>
                                                    </div>
                                                </div>
                                            </section>
                                        </c:if>
                                                ...
                                                <div class="d-flex align-items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="doctor-muted">
                                                            Kết quả xét nghiệm đã có. Hãy chạy AI để mở phần kết luận
                                                            của bác sĩ.
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="col-xl-5">
                                            <section class="doctor-card diagnosis-panel">
                                                <div class="d-flex align-items-center gap-3 mb-3">
                                                    <span class="section-icon"><i
                                                            class="bi bi-journal-medical"></i></span>
                                                    <div>
                                                        <h2 class="doctor-section-title h5 mb-0">Kết quả của bác sĩ</h2>
                                                        <div class="doctor-muted small">${canEditDiagnosis ? 'Có thể cập
                                                            nhật và hoàn thành hồ sơ.' : 'Hồ sơ đang ở chế độ chỉ đọc.'}
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label fw-bold">Ghi chú bác sĩ</label>
                                                    <textarea id="doctorNotes" class="form-control" rows="7"
                                                        ${!canEditDiagnosis ? 'disabled' : ''
                                                        }>${record.doctor_notes}</textarea>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label fw-bold">Chẩn đoán cuối cùng</label>
                                                    <select id="finalDiagnosis" class="form-select" ${!canEditDiagnosis
                                                        ? 'disabled' : '' }>
                                                        <option value="Bình thường"
                                                            ${record.finalDiagnosis=='Bình thường' ? 'selected' : '' }>
                                                            Bình thường</option>
                                                        <option value="Tiền tiểu đường"
                                                            ${record.finalDiagnosis=='Tiền tiểu đường' ? 'selected' : ''
                                                            }>Tiền tiểu đường</option>
                                                        <option value="Tiểu đường Type 1"
                                                            ${record.finalDiagnosis=='Tiểu đường Type 1' or
                                                            record.finalDiagnosis=='Tiểu Đường Type 1' ? 'selected' : ''
                                                            }>Tiểu Đường Type 1</option>
                                                        <option value="Tiểu đường Type 2"
                                                            ${record.finalDiagnosis=='Tiểu đường Type 2' or
                                                            record.finalDiagnosis=='Tiểu Đường Type 2' or
                                                            record.finalDiagnosis=='Tiểu đường' ? 'selected' : '' }>Tiểu
                                                            Đường Type 2</option>
                                                        <option value="Tiểu đường Thai Kỳ"
                                                            ${record.finalDiagnosis=='Tiểu đường Thai Kỳ' or
                                                            record.finalDiagnosis=='Tiểu đường Thai kỳ' ? 'selected'
                                                            : '' }>Tiểu Đường Thai Kỳ</option>
                                                    </select>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label fw-bold">Hẹn ngày tái khám (Không bắt
                                                        buộc)</label>
                                                    <input id="revisitDate" type="date" class="form-control"
                                                        ${!canEditDiagnosis ? 'disabled' : '' }
                                                        value="${record.revisitDateFormatted}">
                                                </div>
                                                <div class="form-check mb-4">
                                                    <input id="canView" class="form-check-input" type="checkbox"
                                                        ${record.canPatientView ? 'checked' : '' } ${!canEditDiagnosis
                                                        ? 'disabled' : '' }>
                                                    <label class="form-check-label" for="canView">Cho phép bệnh nhân xem
                                                        kết quả</label>
                                                </div>
                                                <c:if test="${canEditDiagnosis}">
                                                    <button class="btn btn-doctor w-100" type="button"
                                                        onclick="saveNotes('${record.healthRecordId}')">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Phân tích AI</h5><button class="btn-close"
                                            data-bs-dismiss="modal"></button>
                                    </div>
                                    <div class="modal-body">Hệ thống sẽ gửi các chỉ số xét nghiệm hiện tại sang AI để
                                        phân tích.</div>
                                    <div class="modal-footer">
                                        <form action="${pageContext.request.contextPath}/doctor/ai/process"
                                            method="post">
